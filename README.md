...
- **材料卡片克洛格卡式（V1.7.6）**：材料详情卡片改为纵向信息卡：分类胶囊 → 名称 → 区域/坐标/位置 →
  256px 高清大图 → 用途（升级素材/料理材料/强化材料自动标签）→ 导航/收集/标记完成三按钮；
  「收集」标记当前点位后自动定位到该材料下一个未收集位置（导航程序雏形）；
  「标记完成」为单点级（只标记当前点位，不整材料）；材料会刷新，重新勾选左侧材料名即清除标记完成、全部位置重新显示
- **卡片图标按宽度协调（V1.7.7）**：材料卡片高清图改为 160px（居中）
- **卡片宽度回 300px + 图标 180px + 查看原图（V1.7.8）**：卡片 300px 宽、图标 180px 居中；
  点击图标弹出 256px 原图预览（克洛格卡「查看原图」同款，点击任意处关闭）
